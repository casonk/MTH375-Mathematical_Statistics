...
\begin{center}
  ...
  \color{purple}\textit{\href{https://github.com/casonk/MTH375-Mathematical_Statistics/tree/main/HW/HW1}{github}}\color{black}
\end{center}
\hrulefill
```
...
```{=latex}
\vfill 
    Key Concepts:  Multivariate normal({$\bm{\mu},\Sigma)$, $\chi^2$ with $n$ df, $t$ with $n$ df, $F$ with $m,n$ df
\newpage
```
...
```{r, size='large'}
qnorm(p = c(0.005, 0.995), mean = 0, sd = 29)
```

```{=latex}
\vspace{5mm}
We can see here that $ a = 74.69905 $ is the solution.
```
...
\vspace{2.5mm}
\begin{itemize}
  \item $\displaystyle C\sqrt{6} = \frac{Z_1\sqrt{6}}{\sqrt{Z^2_2+Z^2_3+\cdots+Z^2_6}} \thicksim T_6$
  \vspace{2.5mm}
  \item $\displaystyle P\left( -a < \frac{Z_1}{\sqrt{Z^2_2+Z^2_3+\cdots+Z^2_6}} < a \right) = 
  P\left( -a\sqrt{6} < \frac{Z_1\sqrt{6}}{\sqrt{Z^2_2+Z^2_3+\cdots+Z^2_6}} < a\sqrt{6} \right)$
\end{itemize}
\vspace{5mm}
We now wish to have probabilities for $ -a\sqrt{6} $ corresponding to $ 0.005 $ and $ a\sqrt{6} $ corresponding to $ 0.995 $.

In particular, we will solve $\displaystyle P\left( -a\sqrt{6} < T_6 < a\sqrt{6} \right) = 0.99$. 
\vspace{5mm}
```

```{r, size='large'}
a_sqrt_6 <- qt(p = c(0.005, 0.995), df = 6)
a_sqrt_6
a_sqrt_6 / sqrt(6)
```
...
```{=latex}
\vspace{5mm}
We can see here that $ a = 1.513551 $ is the solution.
```
